...
size_t read_ubyte_dataset(const char *image_filename,
    ...
    ubyte_image_dataset image_header;
    ubyte_label_dataset label_header;

    // Read and verify file headers
    if (fread(&image_header, sizeof(ubyte_image_dataset), 1, imfp) != 1) {
        printf("ERROR: Invalid dataset file (image file header)\n");
        fclose(imfp);
        fclose(lbfp);
        return 0;
    }
    if (fread(&label_header, sizeof(ubyte_label_dataset), 1, lbfp) != 1) {
